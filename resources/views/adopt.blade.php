...
<div class="container">
    ...
    <form class="filters" method="GET" action="{{ route('adopt') }}">
        <select name="breed">
            <option value="">Select Breed</option>
            <option {{ request('breed') == 'Golden Retriever' ? 'selected' : '' }}>Golden Retriever</option>
            <option {{ request('breed') == 'Siberian Husky' ? 'selected' : '' }}>Siberian Husky</option>
            <option {{ request('breed') == 'Persian Cat' ? 'selected' : '' }}>Persian Cat</option>
        </select>

        <select name="age">
            <option value="">Select Age</option>
            <option {{ request('age') == 'Puppy/Kitten' ? 'selected' : '' }}>Puppy/Kitten</option>
            <option {{ request('age') == '1-3 Years' ? 'selected' : '' }}>1-3 Years</option>
            <option {{ request('age') == '4+ Years' ? 'selected' : '' }}>4+ Years</option>
        </select>

        <input type="text" name="location" placeholder="Enter Location" value="{{ request('location') }}">
    ...
    <div class="pets-grid">
        @forelse ($pets as $pet)
            <div class="pet-card">
                ...
                <h3>{{ $pet->name }}</h3>
                <p>Breed: {{ $pet->breed }}<br>Age: {{ $pet->age_group }}<br>Location: {{ $pet->location }}</p>
...
<div id="connectModal">
    <div class="modal-content">
        ...
        <form id="connectForm">
            <input type="text" name="name" id="modalName" value="{{ auth()->user()->name }}" readonly>
            <input type="email" name="email" id="modalEmail" value="{{ auth()->user()->email }}" readonly>
